...
const CheckoutPage = ({ cart, removeFromCart, clearCart, fetchProducts }: any) => {
  ...
  const procesarCompra = async (metodoPago: 'MP' | 'WA') => {
    ...
    try {
      ...
      // Si el backend responde 200 (para WA) o 201 (para MP)
      if (response.status === 200 || response.status === 201) {
        
        // 1. Si es WhatsApp, primero abrimos el link (El backend ya ignoró la venta en DB)
        if (metodoPago === 'WA') {
          const mensaje = `¡Hola! Realicé un pedido:\n${cart.map((i:any) => `- ${i.quantity}x ${i.name}`).join('\n')}\nTotal: $${totalFinal.toLocaleString()}\nEntrega: ${metodoEntrega}`;
          window.open(`https://wa.me/5493794123456?text=${encodeURIComponent(mensaje)}`, '_blank');
        } else {
          alert("✅ ¡PEDIDO RECIBIDO! El Capitán ya está avivando el fuego.");
        }
        
        // 2. Limpiamos el carrito
        clearCart(); 
        
        // 3. Refrescamos productos para actualizar stock visualmente
        if (fetchProducts) {
          await fetchProducts(); 
        }
                ...
                return (
                  <div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: '15px', paddingBottom: '15px', borderBottom: '1px solid rgba(0,0,0,0.1)' }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <p style={{ fontSize: '0.9rem', color: '#D84315', fontWeight: '900', margin: '5px 0' }}>{item.quantity} x ${precioUnitario.toLocaleString()}</p>
                    </div>
